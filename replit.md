...
- **Coach Profile Pages**: Complete public profile pages for each coach accessible via `/coach/{coach_id}`
    - **Design**: Planity-style minimalist design with Inter font, white background, clean borders, and professional layout
    - **Hero Section**: Large profile photo (140px), full name with verified badge, rating with review count, city location, and pricing
    - **Information Sections**:
      - **Spécialités**: Tags displaying coach's specialties (musculation, nutrition, cardio, etc.)
      - **À propos**: Biography section with coach's experience and approach
      - **Expérience**: Professional experience display with client satisfaction count
      - **Salles d'entraînement**: List of gyms where the coach trains with full address details
      - **Réseaux sociaux**: Clickable Instagram and TikTok links opening in new tabs with gradient buttons
      - **Avis clients**: Review cards with client name, star rating, comment, and date
    - **Booking Calendar**: Right-side sticky section with:
      - Month navigation (← Janvier 2025 →)
      - Available time slots displayed in a 2-column grid
      - Green-highlighted available slots (9:00, 10:30, 14:00, 16:30)
      - Grayed-out unavailable slots
      - "Réserver cette séance" button with hover effects
    - **Empty States**: Displays "Ce coach n'a pas encore renseigné..." messages when data is missing (calendar, bio, specialties, reviews)
    - **Navigation**: "← Retour" button in header to return to previous page
    - **Responsive**: 2-column layout (content + booking) on desktop, stacked single column on mobile
    - **404 Handling**: Non-existent coach IDs properly return 404 error page
    - **Data Sources**: Loads coach data from `coaches.json` and demo_users database with fallback support
